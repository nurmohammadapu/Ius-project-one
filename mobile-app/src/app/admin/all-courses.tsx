    ...
    <SafeAreaView className="flex-1 bg-pure-greys-5 dark:bg-richblack-900">
      ...
      <View className="px-4 py-3 border-b border-pure-greys-25 dark:border-richblack-800 flex-row justify-between items-center bg-white dark:bg-richblack-900">
        <View className="flex-row items-center">
          ...
          <Text className="text-xl font-bold text-richblack-900 dark:text-richblack-5">All Courses</Text>
        </View>
      </View>
              ...
              >
                <View className="flex-row items-center mb-3">
                  ...
                  <View className="ml-3 flex-1">
                    ...
                    <Text className="text-xxs text-richblack-500 dark:text-richblack-400">
                ...
                {/* Info row */}
                <View className="flex-row justify-between items-center py-2.5 border-t border-pure-greys-50 dark:border-richblack-700">
                  <Text className="text-xs text-richblack-700 dark:text-richblack-200">Price: ${course.price}</Text>
                  <Text className="text-xs text-richblack-700 dark:text-richblack-200">Students: {course.studentsEnroled?.length || 0}</Text>
                  <View
                ...
                {/* Action buttons */}
                <View className="flex-row justify-end space-x-3 pt-2.5 border-t border-pure-greys-50 dark:border-richblack-700">
                  <TouchableOpacity
                    onPress={() => setViewingCourse(course)}
                    className="bg-pure-greys-5 dark:bg-richblack-900 border border-pure-greys-50 dark:border-richblack-700 px-3 py-1.5 rounded-lg flex-row items-center mr-2"
                  >
                    <Ionicons name="eye-outline" size={16} color="#AFB2BF" />
                    <Text className="text-richblack-700 dark:text-richblack-100 font-bold text-xs ml-1.5">Review</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
      ...
      {viewingCourse && (
        <Modal visible={!!viewingCourse} transparent animationType="slide">
          <View className="flex-1 bg-black/80 justify-center items-center px-4 py-8">
            <ScrollView className="w-full bg-white dark:bg-richblack-800 rounded-2xl border border-pure-greys-25 dark:border-richblack-700 max-h-[90%]" showsVerticalScrollIndicator={false}>
              {/* Modal Header */}
              <View className="flex-row justify-between items-center px-5 py-4 bg-pure-greys-5 dark:bg-richblack-700 rounded-t-2xl border-b border-pure-greys-25 dark:border-b-richblack-600">
                <Text className="text-base font-bold text-richblack-900 dark:text-richblack-5">Course Outline & Review</Text>
              ...
              <View className="p-5">
                {/* Visual outline */}
                <View className="flex-row items-center mb-4 pb-4 border-b border-pure-greys-50 dark:border-richblack-700">
                  <Image
                  ...
                  />
                  <View className="ml-3 flex-1">
                    <Text className="text-base font-bold text-richblack-900 dark:text-richblack-5">{viewingCourse.courseName}</Text>
                    <Text className="text-xs text-richblack-600 dark:text-richblack-300 mt-0.5">
                      Instructor: {viewingCourse.instructor?.firstName} {viewingCourse.instructor?.lastName}
                    </Text>
                    <Text className="text-xs text-richblack-500 dark:text-richblack-400">
                ...
                {/* Description */}
                <View className="mb-4">
                  <Text className="text-xs font-bold text-richblack-600 dark:text-richblack-200 uppercase mb-1">Description</Text>
                ...
                {/* What you will learn */}
                <View className="mb-4">
                  <Text className="text-xs font-bold text-richblack-600 dark:text-richblack-200 uppercase mb-1">What you will learn</Text>
                  <Text className="text-xs text-richblack-700 dark:text-richblack-100 leading-relaxed">
                ...
                {/* Section Accordions */}
                <View className="mb-4">
                  <Text className="text-xs font-bold text-richblack-600 dark:text-richblack-200 uppercase mb-2">Sections & Lectures</Text>
                  {!viewingCourse.courseContent || viewingCourse.courseContent.length === 0 ? (
                    <Text className="text-xs text-richblack-500 dark:text-richblack-400 italic">No content has been uploaded.</Text>
                  ) : (
                    viewingCourse.courseContent.map((section: any, sIdx: number) => (
                      <View key={section.id || sIdx} className="bg-pure-greys-5 dark:bg-richblack-900 border border-pure-greys-50 dark:border-richblack-700 rounded-lg p-3 mb-3">
                        <Text className="text-xs font-bold text-richblack-900 dark:text-richblack-5 mb-1.5">
                          Section {sIdx + 1}: {section.sectionName}
                        </Text>
                        {section.subSection?.map((sub: any, subIdx: number) => (
                          <View key={sub.id || subIdx} className="pl-4 py-1 flex-row justify-between items-center border-l border-pure-greys-50 dark:border-l-richblack-700 mt-1">
                            <Text className="text-xxs text-richblack-700 dark:text-richblack-100 flex-1 mr-2" numberOfLines={1}>
                              {sIdx + 1}.{subIdx + 1} {sub.title}
                            </Text>
                            <Text className="text-xxs text-richblack-500 dark:text-richblack-400 font-mono">{sub.timeDuration}s</Text>
                          </View>
                        ))}
                      </View>
                    ))
                ...
                <TouchableOpacity
                  ...
                  className="bg-pure-greys-25 dark:bg-richblack-700 py-2.5 rounded-lg flex-row justify-center items-center mt-6"
                >
                  <Text className="text-richblack-900 dark:text-richblack-5 font-bold text-xs">Close Review</Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
          </View>
