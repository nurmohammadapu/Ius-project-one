...
export async function getAllStudents(token) {
  ...
  try {
    ...
    Alert.alert("Error", error.response?.data?.message || "Could Not Fetch Students");
  }
  return result;
}
...
export async function getAllInstructors(token) {
  ...
  try {
    ...
    Alert.alert("Error", error.response?.data?.message || "Could Not Fetch Instructors");
  }
  return result;
}
...
export async function getAllCourses(token) {
  ...
  try {
    ...
    Alert.alert("Error", error.response?.data?.message || "Could Not Fetch Courses");
  }
  return result;
}
...
export async function toggleCoursePublish(courseId, publish, token) {
  ...
  try {
    ...
    Alert.alert("Success", response.data.message || "Course status updated successfully");
    success = true;
  } catch (error) {
    console.log("TOGGLE_COURSE_PUBLISH_API ERROR............", error);
    Alert.alert("Error", error.response?.data?.message || "Failed to Update Course Status");
  }
  return success;
}
...
export async function manageInstructor(instructorId, action, token) {
  ...
  try {
    ...
    Alert.alert("Success", `Instructor ${action === "approve" ? "approved" : "denied"} successfully`);
    success = true;
  } catch (error) {
    console.log("MANAGE_INSTRUCTOR_API ERROR............", error);
    Alert.alert("Error", error.response?.data?.message || "Failed to process approval");
  }
  return success;
}
...
export async function getFinancialReport(token) {
  ...
  try {
    ...
    Alert.alert("Error", error.response?.data?.message || "Could Not Fetch Financial Report");
  }
  return result;
}
