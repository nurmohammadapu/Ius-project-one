...
export default function AllStudents() {
  ...
  const handleToggleStatus = async (userId, currentActive) => {
    ...
    if (success) {
      setStudents((prev) =>
        prev.map((student) =>
          student.id === userId ? { ...student, active: !currentActive } : student
        )
      )
    }
  }
  ...
  const handleCreateUserSubmit = async (e) => {
    e.preventDefault()
    const payload = { ...formData, accountType: "Student" }
  ...
  return (
    <div className="mx-auto w-11/12 max-w-[1000px] py-10">
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-medium text-richblack-5">All Students</h1>
          <p className="text-sm text-richblack-400 mt-1">Manage student accounts, registration, active status and details.</p>
        </div>
          ...
          <tbody>
            {filteredStudents.length === 0 ? (
              ...
              filteredStudents.map((student) => (
                <tr key={student.id} className="border-b border-richblack-800 hover:bg-richblack-800/50 transition-all">
                  ...
                  <td className="px-6 py-4">{formatDate(student.createdAt).split(" | ")[0]}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-xl border border-richblack-700 bg-richblack-800 p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-xl font-bold text-richblack-5">Create New Student</h2>
              <button
